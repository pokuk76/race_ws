  ...
  */
  public:
   ...
   void get_omap()
   {
	...
	auto future_result = omap_client_->async_send_request(request);	
	
	if (rclcpp::spin_until_future_complete(Node, future_result) == rclcpp::FutureReturnCode::SUCCESS)
	{
            this->map_msg = future_result.get()->map;
	    ...
	    this->orientation_x = future_result.get()->map.info.origin.orientation.x;
	    this->orientation_y = future_result.get()->map.info.origin.orientation.y;
            this->orientation_z = future_result.get()->map.info.origin.orientation.z;
	    this->orientation_w = future_result.get()->map.info.origin.orientation.w;
	    // this->MAX_RANGE_PX = this->MAX_RANGE_METERS / this->resolution;
	}
